          ...
          {patterns.map((p, i) => {
            ...
            return (
              <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
                ...
                {p.icd10_codes?.length > 0 && <div><p className="text-precision text-[0.6rem] text-clinical-stone uppercase tracking-widest mb-2">ICD-10</p><div className="flex flex-wrap gap-1.5">{p.icd10_codes.map(c => <span key={c} className="text-precision text-[0.6rem] font-medium text-clinical-charcoal bg-surface-container px-2 py-0.5" style={{ borderRadius: '2px' }}>{c}</span>)}</div></div>}
              </motion.div>
            );
          })}
        </div>

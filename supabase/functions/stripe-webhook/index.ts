...
//
// Handles all Stripe lifecycle events:
//   - checkout.session.completed → first-time upgrade (links user to subscription)
...
// Updates subscription_tier='pro' when active, 'free' on cancellation.
    ...
    switch (event.type) {
      ...
      case 'checkout.session.completed': {
        ...
        await admin.from('profiles').update({
          ...
          comp_code_used: null, // they paid, drop any prior comp code
        }).eq('id', userId)
        console.log(`[stripe-webhook] User ${userId} upgraded to Pro via checkout`)
        break
      }

...
import {
  buildTodayActions,
  buildPhaseActions,
  buildFindingExplanations,
  buildPatternDescriptions,
  selectEatingPattern,
...
export interface ClinicalFacts {
  ...
  // ──────────────────────────────────────────────────────────────────────
  // DETERMINISTIC PROSE (2026-05-11-29)
  // ──────────────────────────────────────────────────────────────────────
  // Previously generated by AI; now built from templates in
  // ./rules/proseTemplates.ts. AI prompts receive these as
  // `pre_computed_*` keys and use them verbatim — they don't regenerate.
  // Saves ~$0.10/generation and ensures cross-surface consistency.
  todayActions: TodayAction[];
  actionPlan: ActionPlan;
  findingExplanations: FindingExplanation[];
  patternDescriptions: PatternDescription[];
...
export function buildPlan(input: PatientInput): ClinicalFacts {
  ...
  const factsForSymptoms: ClinicalFacts = {
    ...
    // Prose template placeholders — computed at the very end of buildPlan
    todayActions: [],
    actionPlan: { phase_1: { name:'', focus:'', actions:[] }, phase_2: { name:'', focus:'', actions:[] }, phase_3: { name:'', focus:'', actions:[] } },
    findingExplanations: [],
    patternDescriptions: [],
  ...
  return {
    ...
    canonicalProse: buildCanonicalProse({
      ...
      todayActions: [], actionPlan: { phase_1:{name:'',focus:'',actions:[]}, phase_2:{name:'',focus:'',actions:[]}, phase_3:{name:'',focus:'',actions:[]} },
      findingExplanations: [], patternDescriptions: [],
      eatingPattern: { name:'', rationale:'', emphasize:[], limit:[] },
      lifestyleInterventions: { diet:[], sleep:[], exercise:[], stress:[] },
    ...
    // The functions only read fields above (patient, labs, conditions,
    // supplementCandidates, depletions, expectedFindings), all final.
    // ────────────────────────────────────────────────────────────────────
    ...(() => {
      // Build a temporary facts shape with empty prose so templates can read.
      const tmpFacts: ClinicalFacts = {
        patient: {
          age: input.age, sex: sexNormalized,
          heightCm: input.heightCm, weightKg: input.weightKg,
          bmi: input.bmi, bmiCategory: bmiCategoryFor(input.bmi),
        ...
        riskCalculators, emergencyAlerts, crisisAlert, prepInstructions,
        suboptimalFlags, goalTargets, symptomsAddressed,
        citations, isOptimizationMode, expectedFindings,
        canonicalProse: { conditions: [], outliers: [], supplements: [], goals: [], alerts: [] },
        todayActions: [], actionPlan: { phase_1:{name:'',focus:'',actions:[]}, phase_2:{name:'',focus:'',actions:[]}, phase_3:{name:'',focus:'',actions:[]} },
        findingExplanations: [], patternDescriptions: [],
        eatingPattern: { name:'', rationale:'', emphasize:[], limit:[] },
      ...
      return {
        ...
        actionPlan: buildPhaseActions(tmpFacts),
        findingExplanations: buildFindingExplanations(tmpFacts),
        patternDescriptions: buildPatternDescriptions(tmpFacts),
        eatingPattern: selectEatingPattern(tmpFacts),
